...
本节界定产品侧法条引用（`app/statutory_retrieval.py`）对外承诺的范围，供验收与集成方判断可用性。产品路径只有三种状态，不存在第四种：

| 状态 | 触发条件 | 含义 |
| --- | --- | --- |
| `ok` | 问题中显式写出法名与条号，且在受控语料中**唯一法名 + 唯一条号**命中 | 已核验引用；结果附版本与来源 |
| `candidate` | 问题未给出显式引用，转为对语料的文本检索 | 相关条文候选，**不构成已核验引用** |
| `needs_review` | 语料未配置/完整性问题、引用无法唯一解析、版本歧义、无命中 | 拒绝核验，交律师处理 |

### 能保证

- **显式引用精确落到指定法律与条号。** `ok` 只能由精确查找产生；百/千位条号（第二百六十四条、第一千零六十条）按中文数字完整解析，不会被截断为低位条号。
- **文本检索永不升级为引用。** 词面相近的段落只返回 `candidate`，因为语义近似不等于法律引用成立。
- **绝不使用相近法条替代。** 点名法律不在语料中、条号不存在、或两部不同法律共用同一简称时，返回 `needs_review`，且返回结果中不含被点名法律之外的条文。
- **版本歧义即拒答。** 同一部法存在多个版本而未指定版本时，产品层返回 `needs_review`（内部由 `VersionAmbiguityError` 触发），不静默择一。
- **可追溯。** 每条结果携带法名、条号、版本日期、版本状态、来源 URL、文书 ID 与正文，可回看官方法源。
- **语料完整性可验证。** 加载时逐份校验文书 SHA-256 与 schema 版本，失配即整份不可用；语料缺失或损坏时状态为 `needs_review` 且 `hits` 为空，不会退回模型记忆作答。就绪探针 `/api/ready` 以 `legal_verification: available/unavailable` 反映该状态。

上述行为的回归用例集中在 `tests/test_statutory_retrieval.py`，含条号截断、法名重名、点名法律缺席（如“工伤保险条例第十四条”在未收录该法时）等历史误报场景：

```sh
uv run --locked pytest tests/test_statutory_retrieval.py --disable-socket --allow-unix-socket
```

### 不能保证

- **不保证引用的是现行有效法条。** 语料的 `version_status` 标注为 `currentness_not_asserted`：系统只保证在受控语料中精确、唯一地定位到**某一版本**条文并标注其版本与来源，是否现行有效须由律师核验。历史文本（如被 2017 年修订替代的合作社法）与经修订文本并存时尤其如此。
- **不保证正文的逐字法律准确性。** 哈希只证明正文与已保存的官方法源文本一致，不证明官方转载本身无错；官方域名不等于转载绝无错误。
- **不证明条文适用于本案。** 检索只回答“该条是否存在并已定位”，不回答“该条是否适用于当前事实”。场景类问题的适用性判断仍由生成模型与律师完成。
- **不保证覆盖范围。** 语料是受控子集。点名法律缺收时返回 `needs_review`，这只是“本系统无法核验”，**不是**“该法不存在或已失效”的结论。
- **不解析未写成规范引用的表达。** 条文引用须匹配《法名》第X条形态；“本法第四十条”这类回指、以及“民法典第N条”这类无效条号不会被解析为引用，同一条的款/项细节不参与定位（条号级）。
- **不保证整个生成链路都不作答。** fail-closed 仅适用于法条引用路径：法条不可核验时拒答并标记待核验；案件卷宗检索不足时仍降级为基于已检索材料的规则式回答，并提示律师补充材料。
