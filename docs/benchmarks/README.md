...
统一 runner 仅对 2-1 成功响应在评分前应用 `preserve_correction_surface`（只读取题面与输出，恢复源句标点、空白及句尾形式）；不启用其他任务后处理。`prediction` 是评分文本，`original_prediction` 始终保留该任务的模型返回文本，`postprocess` 保存处理版本、策略与是否变更。manifest 冻结任务范围、版本和后处理源码哈希；旧 manifest 或缺失处理溯源的 2-1 检查点不能续跑为新配置，请使用新运行目录。评分器与提示词未改变，离线回放收益不代表新模型运行或官方 LawBench 成绩。
