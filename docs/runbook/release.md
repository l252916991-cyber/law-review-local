...
浏览器登录签发独立随机会话，不在 cookie 中保存配置访问令牌。SQLite 以哈希保存会话标识及凭证指纹/组织 subject，8 小时到期、退出立即撤销，Web 进程重启后仍可恢复；每次请求重新解析当前授权，删除或轮换配置令牌即拒绝旧会话。会话上限 4096，不是集中身份服务；Bearer API 令牌仍由部署配置管理，应通过安全配置更新执行轮换，不能用浏览器退出代替撤销 API 凭证。
