...
当前默认策略与实验边界见 [2026-09-14 策略基线](docs/architecture/strategy-baseline-20260914.md)，后续优化以该基线开展对照。
...
- **升级**：SQLite schema 有序迁移（当前 v13），拒绝未来版本；跨版本升级前先做备份，迁移失败自动回滚。v13 增加按父页级联删除的页内子块缓存，只保留每页当前向量空间。
